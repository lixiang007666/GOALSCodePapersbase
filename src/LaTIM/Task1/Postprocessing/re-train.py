...
class OCTDataset(Dataset):
    ...
    def transform(self,img, mask):
        (d, t, sc, sh) = transforms.RandomAffine.get_params(degrees=(-50, 50), translate=(0.5, 0.5),
                                                            scale_ranges=(0.6, 1.4), shears=(-50, 50),
                                                            img_size=img.shape)
...
best_model_path = "model623_1.0667138594488792.pth"
...
metric2 = monai.metrics.SurfaceDistanceMetric(include_background=True,reduction="mean")
...
for epoch in range(num_epochs):
    ...
    with torch.enable_grad():
        for batch_idx, data in enumerate(train_loader):
            ...
            out = monai.networks.utils.one_hot(pred_img.view(pred_img.shape[0], 1, image_size, image_size2),
                                               num_classes=4, dim=1)
            oh_gt = monai.networks.utils.one_hot(gt_label, num_classes=4, dim=1)
            score = metric2(out, oh_gt)
            ...
            loss = criterion(logits, torch.squeeze(gt_label, dim=1).long()) + dice + 0.1 * score
            metric2.reset()
