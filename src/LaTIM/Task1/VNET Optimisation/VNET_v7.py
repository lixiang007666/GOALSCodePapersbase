...
class OCTDataset(Dataset):
    def __init__(self, image_file, gt_path=None, filelists=None,  mode='train' , transform_valeur = 20):
        ...
        self.transform_valeur = int(transform_valeur)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            #print(self.transform_valeur)
            #print(self.transform_valeur/100)
            (d, t, sc, sh) = transforms.RandomAffine.get_params(degrees=(-self.transform_valeur, self.transform_valeur), translate=((self.transform_valeur/100), (self.transform_valeur/100)),
                                                    scale_ranges=((1-self.transform_valeur/100), (1+(self.transform_valeur/100))), shears=(-self.transform_valeur, self.transform_valeur),
                                                    img_size=img.shape)
            img = affine(to_pil_image(img), angle=d, translate=t, scale=sc, shear=sh)
            gt_img = affine(to_pil_image(gt_img), angle=d, translate=t, scale=sc, shear=sh)
            img = np.array(img)
            ...
            deform = Rand2DElastic(
                prob=(self.transform_valeur/100+0.1),
                spacing=(self.transform_valeur, self.transform_valeur),
                magnitude_range=(5, 6),
                rotate_range=(np.pi / 4,),
                scale_range=(self.transform_valeur/100, self.transform_valeur/100),
...
def objective(trial):
    ...
    best_model_path = "model563_1.072693606666316.pth"
    ...
    aug_valeur = trial.suggest_int("aug_valeur", 10, 80)
    train_dataset = OCTDataset(image_file = images_file, 
                        gt_path = gt_file,
                        filelists=filelists,transform_valeur = aug_valeur)

    val_dataset = OCTDataset(image_file = images_file, 
                            gt_path = gt_file,
                            filelists=filelists,mode='val')
    ...
    for epoch in range(num_epochs):
        ...
        model.eval()
        critere = monai.metrics.SurfaceDistanceMetric(include_background=False,reduction="mean")
        val_avg_list = []
        with torch.no_grad():
            for batch_idx, data in enumerate(val_loader):
                ...
                out = monai.networks.utils.one_hot(pred_img.view(1,1,image_size,image_size2), num_classes=4, dim=1)
                #print(out.shape)
        ...
        if val_distance < best_score:
            torch.save(model.state_dict(), path)
            best_score = val_distance
            print('##### best model updata ! now best_score = ',best_score)

        
        trial.report(val_distance, epoch)
