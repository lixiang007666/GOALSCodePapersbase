...
import argparse
from loss import LabelSmoothingCrossEntropyLoss as lsceloss
...
def train(model, iters, train_dataloader, val_dataloader, optimizer, criterion, metric, focalloss, vggloss, log_interval, evl_interval, device, fold):
    vggloss.eval()
    # vggloss.to(device)
    ...
    while iter < iters:
        for _, data in enumerate(train_dataloader):
            ...
            # loss = criterion(logits, gt_label) + dice_loss + 0.01*vggloss(logits, gt_label)
            ...
            if iter % evl_interval == 0:
                ...
                if avg_dice >= best_dice:
                    best_dice = avg_dice
                    torch.save(model.state_dict(), '/home/zhouziyu/miccai2022challenge/GOALS_code/checkpoints/task1/msnet_imgaugmore_trainvalClearlySplit_focalCEandDiceloss_fold{}.pth'.format(fold))
...
metric = DiceLoss().to(device)
vggloss = LossNet(device=device).to(device)
...
# 开始训练
train(model, iters, train_loader, val_loader, optimizer, criterion, metric, focalloss, vggloss, log_interval=10, evl_interval=50, device=device, fold=args.fold)
