    ...
    while iter < iters:
        for _, data in enumerate(train_dataloader):
            ...
            if iter % evl_interval == 0:
                avg_maskloss, avg_dice, val_dice_score, avg_layerloss = val(model, val_dataloader, criterion, metric, l1loss, device)
                print("[EVAL] iter={}/{} avg_maskloss={:.4f} avg_layerloss={:.4f} val_dice_score={}".format(iter, iters, avg_maskloss, avg_layerloss, val_dice_score))
                # if avg_dice >= best_dice:
                #     best_dice = avg_dice
                if avg_layerloss <= min_vall1loss:
                    min_vall1loss = avg_layerloss
                    torch.save(model.state_dict(), '/home/zhouziyu/miccai2022challenge/GOALS_code/checkpoints/task1/cenetECA_trainvalClearlySplit_masklayer_minvall1loss_fold{}.pth'.format(fold))

                model.train()

# 验证函数
def val(model, val_dataloader, criterion, metric, l1loss, device):
    model.eval()
    avg_maskloss_list = []
    avg_layerloss_list = []
    avg_dice_list = []
    with torch.no_grad():
        for data in val_dataloader:
            ...
            logits_layer = pred[:,4:9,:,:] # batchsize*5*800*800
            logits_layer = F.softmax(logits_layer, dim=2) # batchsize*5*800*800
            col_num = torch.unsqueeze(torch.tensor([x for x in range(1,801)]), dim=1)
            col_num = col_num.repeat(gt_label.size()[0],5,1,800).to(device) # 800*1 --> 8*5*800*800
            predict_layer = torch.sum(torch.mul(logits_layer, col_num), dim=2).unsqueeze(dim=2) # 8*5*800
            ...
            avg_layerloss_list.append(l1loss(predict_layer, gt_layer).cpu().detach().numpy())
            avg_maskloss_list.append(loss.cpu().detach().numpy())
            avg_dice_list.append(dice_score.cpu().detach().numpy())

    avg_layerloss = np.array(avg_layerloss_list).mean()
    avg_maskloss = np.array(avg_maskloss_list).mean() # list转array
    avg_dice = np.array(avg_dice_list).mean()

    return avg_maskloss, avg_dice, dice_score, avg_layerloss
