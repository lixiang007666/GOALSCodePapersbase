...
### 设置参数
images_file = '/xx/Train/Image'  # 训练图像路径
gt_file = '/xx/Train/Layer_Masks/'
...
_train = OCTDataset(image_file = images_file, 
                        gt_path = gt_file)

_val = OCTDataset(image_file = images_file, 
                        gt_path = gt_file)

### 在_train数据集中给出5幅图像和相应的金标准, 金标准里分为4类:0-RNFL, 1-GCIPL, 2-脉络膜，3-其他
...
# 预测阶段
# 加载模型参数
test_file = '/xx/Validation/Image'  # 测试图像路径
best_model_path = "/xx/task2_models/xx/model.pdparams"
model = OCT_Layer_UNet(num_classes = 4)
...
for img, idx, h, w in test_dataset:
    ...
    # print(pred_.shape)
    cv2.imwrite('/xx/Layer_Segmentations-val/'+idx, pred_)