...
image_file = '../datasets/Train/Image_size400'  # aug2一张800*1100裁成两张800*800
gt_file = '../datasets/Train/Layer_Masks_size400'
...
def train(model, iters, train_dataloader, val_dataloader, optimizer, criterion, metric, log_interval, evl_interval, device, fold):
    iter = 0
    model.train()
    avg_loss_list = []
    avg_dice_list = []
    ...
    while iter < iters:
        for _, data in enumerate(train_dataloader):
            ...
            loss = 2*criterion(logits, gt_label)+dice_loss
            ...
            if iter % evl_interval == 0:
                ...
                if avg_dice >= best_dice:
                    best_dice = avg_dice
                    torch.save(model.state_dict(), '/home/zhouziyu/miccai2022challenge/GOALS_code/checkpoints/task1/train_cenetECA_size400_batchsize16_aug4_CEandDiceloss_fold{}.pth'.format(fold))
...
# 开始训练
train(model, iters, train_loader, val_loader, optimizer, criterion, metric, log_interval=10, evl_interval=50, device=device, fold=args.fold)
